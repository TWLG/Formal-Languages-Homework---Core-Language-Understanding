...
public class JiffyParser {
	...
	private TreeNode parseIfElseStatement() throws JiffyError {
		...
		if (!(lexer.nextToken() == JiffyLexer.IF_TOKEN)) {
			match(JiffyLexer.IF_TOKEN);
			result = new TreeNode("if", result, parseArithmeticExpression());
		}
	...
	/**
	 * Parse an add/sub expression. If any error occurs we return immediately.
	 *
